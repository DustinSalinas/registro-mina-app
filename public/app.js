...
window.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Listener para la cédula (se dispara al escribir) ---
    document.getElementById('cedula').addEventListener('input', function() {
        const cedula = this.value.trim();
        if (cedula.length === 10) {
            // Siempre busca sin filtrar por fecha (null)
            autocompletarPorCedula(cedula, null);
        }
    });
    ...
    document.getElementById('fecha').addEventListener('change', function() {
        ...
        if (cedula.length === 10) {
            // Aquí SÍ se filtra por la fecha seleccionada
            autocompletarPorCedula(cedula, this.value);
        }
    });
...
/* =============================================
   ...
   - Si fecha = null  → busca el último registro sin importar la fecha
   - Si fecha != null → busca el último registro de esa fecha
   ============================================= */
async function autocompletarPorCedula(cedula, fecha = null) {
    ...
    try {
        ...
        if (data) {
            ...
            // No hay datos para esta cédula (con o sin fecha)
            limpiarCampos();
        }
    } catch (e) {
        console.error('Error en autocompletarPorCedula:', e);
    } finally {
...
async function autocompletarPorPlaca(placa) {
    try {
        ...
        console.error('Error en autocompletarPorPlaca:', e);
    }
}
...
// Formulario de creación de usuario (IDs del HTML: uCedula, uNombres, uPassword, uRol)
...
function abrirEditarUsuario(cedula) {
    ...
    document.getElementById('editCedulaOriginal').value = datos.cedula;
    document.getElementById('editCedulaNueva').value = datos.cedula;
    document.getElementById('editNombres').value = datos.nombres;
    document.getElementById('editRol').value = datos.rol;
...
async function guardarEdicionUsuario() {
    ...
    const nuevas = {
        cedula: document.getElementById('editCedulaNueva').value.trim() || cedulaOriginal,
...
function generarPDF() {
    ...
    doc.text('CONTROL DE ACCESO - REGISTRO DE MINA', pageWidth / 2, 20, { align: 'center' });
    doc.setFontSize(10);
    doc.setTextColor(100);
    doc.text(`Generado: ${new Date().toLocaleString()}`, pageWidth / 2, 28, { align: 'center' });

    // Obtener datos filtrados (usar los mismos que se muestran en la tabla)
    const query = document.getElementById('searchInput').value.toLowerCase();
    let desde = document.getElementById('fechaDesde').value;
    let hasta = document.getElementById('fechaHasta').value;
    if (!desde && !hasta) {
    ...
    if (filtrados.length === 0) {
        alert('No hay registros para generar PDF con los filtros actuales.');
        return;
    }

    const columnas = ['ID', 'Fecha', 'Hora', 'Categoría', 'Movimiento', 'Cédula', 'Nombres', 'Placa', 'Destino', 'Razón'];
    ...
    doc.save('registros_acceso_mina.pdf');
}